        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <h1 class="text-2xl font-semibold mb-5 uppercase">{{ __('Categories') }}</h1>
            ...
            <div class="container mt-8 flex">
                <div id="products-filter-panel" class="mr-5 flex flex-col min-w-[180px] items-stretch">
                    ...
                    <div class="py-5 border-b-gray-500 border-b-2">
                        ...
                        <div class="flex flex-col items-start">
                            <a href="{{ route('products.index', array_merge(request()->all(), ['ratingFilter' => 5])) }}"
                            ...
                            <a href="{{ route('products.index', array_merge(request()->all(), ['ratingFilter' => 4])) }}"
                            ...
                            <a href="{{ route('products.index', array_merge(request()->all(), ['ratingFilter' => 3])) }}"
                            ...
                            <a href="{{ route('products.index', array_merge(request()->all(), ['ratingFilter' => 2])) }}"
                            ...
                            <a href="{{ route('products.index', array_merge(request()->all(), ['ratingFilter' => 1])) }}"
                    ...
                    <div class="py-5 border-b-gray-500 border-b-2">
                        ...
                        <form action="{{ route('products.index', array_merge(request()->all())) }}" method="GET">
                    ...
                    <div class="py-5">
                        <a href="{{ route('products.index', array_merge(request()->except(['ratingFilter', 'minPrice', 'maxPrice']))) }}"
                ...
                <div class="flex flex-col">
                    @if ($currentCategory)
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-6 bg-white border-b border-gray-200">
                                <h1 class="text-2xl font-semibold mb-5 uppercase">{{ $currentCategory->name }}</h1>
                                <p class="mt-2 text-md max-w-[600px]">{{ $currentCategory->description }}</p>
                            </div>
                        </div>
                    @endif
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-3 bg-white border-b border-gray-200 flex items-center space-x-2">
                            ...
                            <div class="flex justify-start items-stretch flex-1 space-x-2">
                                <a href="{{ route('products.index', array_merge(request()->except('sort'), ['sortBy' => 'name'])) }}" 
                                    class="py-2 px-3 @if(request('sortBy') == 'name') bg-green-300 @else bg-white @endif shadow-sm border border-gray-200">
                                    {{ __('Popular') }}
                                </a>
                                <a href="{{ route('products.index', array_merge(request()->except('sort'), ['sortBy' => 'created_at'])) }}" 
                                    class="py-2 px-3 @if(request('sortBy') == 'created_at') bg-green-300 @else bg-white @endif shadow-sm border border-gray-200">
                                    {{ __('Latest') }}
                                </a>
                                <a href="{{ route('products.index', array_merge(request()->all(), ['sortBy' => 'price', 'sort' => 'asc'])) }}" 
                                ...
                                <a href="{{ route('products.index', array_merge(request()->all(), ['sortBy' => 'price', 'sort' => 'desc'])) }}" 
